...
class Products():
    ...
    def convertEspecialCarToDictValues(self,path, row, sheets, listOfColumns):
        ...
        for keys, values in dataRow.items():
            valuesStr = str(values)
            valuesConvert = valuesStr.replace(',' , '.')
            dataRowList.append({listOfColumns[teste] : valuesConvert})
            teste+=1
    ...
    def addOracle(self, path, sheets, tableName):
        ...
        for row in range(1):
            print("dict\n")
            print(stringsInput[0])
            self.convertEspecialCarToDictValues(path, 1, sheets, stringsInput)
